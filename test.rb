...
module GoogleSheetHelper
  ...
  def each
    self.rows.flatten.each { |cell| yield cell }
  end
...
module ColumnExtensions
  ...
  def avg
    ...
    # Filtriranje nultih i praznih vrednosti, i konverzija u float
    filtrirani_podaci = select { |cell| !cell.to_s.strip.empty? && cell.to_f != 0.0 }.map(&:to_f)

    # Ako nema podataka nakon filtriranja, vratiti 0.0
    return 0.0 if filtrirani_podaci.empty?

    # Računanje sume i proseka
    sum = filtrirani_podaci.sum
    sum / filtrirani_podaci.length
  end
...
ws = session.spreadsheet_by_key("178X-q82LSIcfM7mVvrtCLwDRFlRTHEsomh7BhhDuJUo").worksheets[0]
